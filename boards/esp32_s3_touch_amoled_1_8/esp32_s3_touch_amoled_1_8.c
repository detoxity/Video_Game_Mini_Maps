...
esp_err_t bsp_sdcard_mount(void)
{
    ...
    // high-speed mode: 40MHz (the driver falls back if the card refuses);
    // 4-bit is not an option - the board only wires D0
...
{
    /* GRAM was cleared to black during init, so lighting up immediately
     * is safe and gives instant visual feedback */
...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    // NOTE: 50MHz QSPI was tried and is marginal on this wiring - the display
    // pipeline can hang under heavy flush traffic (map scrolling), freezing
    // the LVGL task. 40MHz is the stable maximum; it wasn't the FPS limiter.
...
/* fill the panel RAM with black so nothing random shows when the panel
 * lights up (it powers on with undefined contents) */
static void bsp_display_clear_gram(void)
{
    const int strip_h = 16;
    size_t strip_bytes = (size_t)BSP_LCD_H_RES * strip_h * 2;
    uint16_t *strip = heap_caps_calloc(1, strip_bytes, MALLOC_CAP_DMA);
    if (!strip) return;
    for (int y = 0; y < BSP_LCD_V_RES; y += strip_h) {
        int h = (y + strip_h > BSP_LCD_V_RES) ? BSP_LCD_V_RES - y : strip_h;
        esp_lcd_panel_draw_bitmap(panel_handle, 0, y, BSP_LCD_H_RES, y + h, strip);
    }
    free(strip);
}
...
static lv_display_t *bsp_display_lcd_init()
{
    ...
    const lvgl_port_display_cfg_t disp_cfg = {
        ...
        .double_buffer = true,   // render the next strip while DMA flushes
        ...
        .flags = {
            .sw_rotate = false,   // hardware-only policy; sw path costs every flush
            .buff_dma = true,     // internal DMA-capable draw buffers
    ...
    // The vendor BSP called lvgl_port_add_disp_rgb() here, which is wrong for
    // this QSPI panel on two counts:
    //  - it registers RGB-panel callbacks on the CO5300 handle via a blind
    //    __containerof cast, writing past the small SPI-panel struct and
    //    corrupting adjacent heap (which object dies depends on heap layout -
    //    the "random freeze after the map starts animating" bug)
    //  - it marks the display as TYPE_RGB, so lv_disp_flush_ready() fires
    //    immediately after the ASYNC draw_bitmap: LVGL reuses the draw buffer
    //    while the QSPI DMA is still reading it (no real flush handshake)
    // lvgl_port_add_disp() is the correct path for SPI/QSPI panels: it hooks
    // on_color_trans_done on the IO handle, so flush-ready comes from the
    // actual DMA-done interrupt and double-buffering pipelines safely.
...
{
    /* The touch controller shares its reset line with the LCD panel and
     * needs time to boot after the panel reset - a single probe right after
     * LCD init fails intermittently. Retry a few times. */
    ...
    if (err != ESP_OK || tp == NULL) {
        ESP_LOGW(TAG, "Touch controller not responding - continuing without touch input");
        return NULL;
    }
...
/* Background recovery: some boots the touch chip stays unresponsive well
 * past display init. Keep probing and attach the input device when (if)
 * it finally wakes up, so the UI is never stuck without touch until the
 * next power cycle. */
static void touch_retry_task(void *param)
{
    lv_display_t *disp = (lv_display_t *)param;
    for (int i = 0; i < 20 && disp_indev == NULL; i++) {
        ...
        if (bsp_touch_new(NULL, &tp) == ESP_OK && tp != NULL) {
            ...
            lvgl_port_unlock();
            ESP_LOGI(TAG, "Touch controller recovered after %d s", i + 1);
            break;
        }
    }
    vTaskDelete(NULL);
...
lv_display_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    /* missing touch must not kill the display */
    disp_indev = bsp_display_indev_init(disp);
    if (disp_indev == NULL) {
        xTaskCreate(touch_retry_task, "touch_retry", 4096, disp, 3, NULL);
    }
...
{
    // The CO5300's MADCTL mirror does not remap partial-update addressing
    // correctly (verified on hardware: full corruption when mirrored, with
    // or without GRAM gap compensation - which is also why Waveshare's own
    // bsp_display_rotate falls back to LVGL software rotation). Hardware
    // 180-degree rotation is therefore not available on this panel; report
    // that honestly so the auto-rotate feature disables itself.
    (void)flip;
